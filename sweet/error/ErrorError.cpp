...
// This file renamed because VS 2008 can't handle files with same names
...
{
    SWEET_ASSERT( buffer );
    std::error_code ec = std::make_error_code(static_cast<ntl::win::win32error>(oserror));
    const std::string msg = ec.message();
    std::strncpy(buffer, msg.c_str(), length);
